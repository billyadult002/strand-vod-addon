...
app.use(async (req, res, next) => {
  ...
  if (url.includes('/catalog/')) {
    try {
      ...
      const idPart = parts[1] ? parts[1].replace('.json', '') : 'maccms_movie';
      const extraStr = parts[2] ? parts[2].replace('.json', '') : '';
      const extraParams = {};
      if (extraStr.includes('search=')) {
        const match = extraStr.match(/search=([^&]+)/);
        if (match) extraParams.search = decodeURIComponent(match[1]);
      }
      const result = await handleCatalog(type, idPart, extraParams);
  ...
  if (url.includes('/meta/')) {
    try {
      ...
      const type = parts[0] || 'movie';
      const idPart = parts[1] ? parts[1].replace('.json', '') : '';
      const result = await handleMeta(type, idPart);
      return res.json(result);
    } catch (e) {
      return res.json({ meta: { id: 'unknown', name: '未知' } });
    }
  }
  ...
  if (url.includes('/stream/')) {
    try {
      ...
      const type = parts[0] || 'movie';
      const idPart = parts[1] ? parts[1].replace('.json', '') : '';
      const result = await handleStream(type, idPart);
      return res.json(result);
    } catch (e) {
  ...
  if (url === '/' || url.endsWith('/src/server.js') || url.endsWith('/api/index.js')) {
